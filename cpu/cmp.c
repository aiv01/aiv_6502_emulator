...
//CMP ($F6,X)	== A - contents of address at $F6 + X
static int cmp_indirect_x(mos6502_t *cpu){   
    return 0;
}
//CMP ($F6),Y   == A - contents of (address at $F6) + offset Y
...
void mos6502_register_cmp(mos6502_t *cpu){
    ...
    mos6502_register_opcode(cpu, 0xC1, cmp_indirect_x);
    mos6502_register_opcode(cpu, 0xD1, cmp_indirect_x);
}
